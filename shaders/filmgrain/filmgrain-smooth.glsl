...
//!HOOK LUMA
//!BIND HOOKED
//!DESC  Film Grain Gaussian Smoothed
//!COMPUTE 32 32
const uint row_size = 2 * fgs_taps + 1;
...
const uvec2 isize = uvec2(gl_WorkGroupSize) + uvec2(2 * fgs_taps);
...
void hook()
{
    ...
    for (uint y = gl_LocalInvocationID.y; y < isize.y; y += gl_WorkGroupSize.y) {
        ...
        // update grain LUT
        grain[y][gl_LocalInvocationID.x + fgs_taps] = hsum;
    }
    ...
    for (uint y = 0; y < row_size; y++) {
        float g = grain[gl_LocalInvocationID.y + y][gl_LocalInvocationID.x + fgs_taps];
        vsum += weights[y] * g;
    }

    vec4 color = HOOKED_tex(HOOKED_pos);
    color.rgb += vec3(fgs_intensity * vsum);
